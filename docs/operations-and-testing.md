...
1. Provision one API host and PostgreSQL 18 with separate runtime/migration credentials. Pin actual dependency versions in implementation lockfiles; set secrets in environment/secret storage (see `.env.example`). Persist protected Data Protection keys and enable encrypted disks. The local Compose scaffold now automatically builds the Nginx proxy, runs PostgreSQL 18, and uses a `db-migrator` container to execute migrations using an admin role before the `backend` starts with a restricted application role. HTTPS configuration on the Nginx proxy should be finalized before using it for public deployment. Do not mount a PostgreSQL 16 data directory into PostgreSQL 18; use a tested dump/restore migration if data exists.
2. Apply migrations using the migration role (handled automatically by the `db-migrator` container); seed the current-model administrator and optional demo users via protected environment settings; association/membership fixtures remain unimplemented under #73. Start API with runtime role (handled by the `backend` container), then run smoke login/creation/isolation tests through the Nginx reverse proxy.
