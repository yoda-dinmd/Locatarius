...
2. Apply migrations using the migration role (handled automatically by the `db-migrator` container); seed synthetic associations and distinct users via the automated seeder reading secrets from the environment. Start API with runtime role (handled by the `backend` container), then run smoke login/creation/isolation tests through the Nginx reverse proxy.
