...
1. For local development, use the [HTTPS runbook](local-https.md): nginx owns loopback HTTPS, frontend/backend are internal, and PostgreSQL retains the existing loopback DB-tools exception. A provisioner maintains bootstrap-separated migration/runtime identities, the non-superuser migrator executes backend-owned migrations/seeding, and a post-migration job applies explicit business-table grants excluding EF history. Persisted Data Protection keys are locally permission-protected; encrypted production keys/disks and public certificate renewal remain deployment requirements. Preserve PostgreSQL 16 volumes and use tested dump/restore rather than mounting their data into 18.
2. Verify `scripts/smoke-local.py --recreate`: HTTPS, SPA routing, secure cookies, CSRF, restricted login, logout/replay, spoofed headers, throttling, persistence and database privileges. `/health/ready` queries migrated tables using the runtime role. Associations/memberships and full UI-to-API/password-change integration remain #73/#77/#83/#87/#88 work; this smoke suite does not establish tenant isolation or full story completion.
